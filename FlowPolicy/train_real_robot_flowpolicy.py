#!/usr/bin/env python3
"""
Real Robot FlowPolicy Training with AgentView Point Cloud Only.
...
class RealRobotFlowPolicy(FlowPolicyWithActionMSE):
    """Real Robot FlowPolicy with enhanced CFM fixes."""
    
    def __init__(self, use_action_mse=True, log_dir=None, **kwargs):
        print("🔧 Initializing RealRobotFlowPolicy with AgentView point cloud...")
...
class RealRobotDataset(BaseDataset):
    ...
    def __init__(self, 
        ...
        self.pc_data = np.array(z['data']['agentview_point_cloud'][:]).astype(np.float32)  # (T, N, 6)
        ...
        print(f"   • AgentView point cloud shape: {self.pc_data.shape}")
        ...
        print(f"   • Number of episodes: {len(self.episode_ends)}")
        print(f"   • Episode lengths: {self.episode_ends - self.episode_starts}")
        
        # Calculate RAM usage
        ram_gb = (self.pc_data.nbytes + self.state_data.nbytes + self.action_data.nbytes) / (1024**3)
    ...
    def __getitem__(self, idx):
        ...
        obs_start = data_idx
        obs_end = obs_start + self.n_obs_steps
        point_cloud = self.pc_data[obs_start:obs_end]  # Already float32 from cache
...
def create_config():
    """Create configuration for real robot training following exact pattern."""
    shape_meta = {
        'obs': {
    ...
    cfg = OmegaConf.create({
        'name': 'train_real_robot_agentview',
        ...
        'policy': {
            '_target_': '__main__.RealRobotFlowPolicy',  # Uses enhanced FlowPolicy with CFM fixes
            'use_action_mse': True,
            'log_dir': '${oc.env:PWD}/FlowPolicy/red_kitkat_pick_place_real_robot_agentview_training_checkpoint/loss_logs',
        ...
        'task': {
            'dataset': {
                ...
                'zarr_path': '/Riad/FlowPolicy/red_kitkat_pick_and_place.zarr',
                'horizon': 4,
                'n_obs_steps': 2,
                'n_action_steps': 4
            },
        ...
        'dataloader': {
            'batch_size': 64,  # Smaller batches for faster loading
            'shuffle': True,
...
if __name__ == "__main__":
    ...
    output_dir = pathlib.Path(__file__).parent / 'red_kitkat_pick_place_real_robot_agentview_training_checkpoint'
    output_dir.mkdir(exist_ok=True)

    print("🚀 Starting Real Robot FlowPolicy Training with NO Action Normalization...")
    ...
    print("   ✅ AgentView point cloud only")
